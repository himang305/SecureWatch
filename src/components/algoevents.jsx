...
function AlgoEvents() {
  ...
  const handleArgumentChange = (event, eventName) => {
    ...
    //             const argsObject = eventDetailsEntry.args.split(",").map((arg) => arg.trim());

    //             const body = {
    //                 name: eventName,
    //                 mid: m_id, // Changed from mid to m_id
    //                 arguments: argsObject,
    //             };

    //             try {
    //                 const response = await axios.post("http://localhost:5000/api/add_event", body);
